...
const Modal = ({ closeModal, modalData }: {closeModal: () => void, modalData: ModalData}) => {
    ...
    return(
        ...
        <label className="flex items-center cursor-pointer relative mx-2">
        <input type="checkbox" onInput={() => setIsChecked(!isChecked)} className="peer h-5 w-5 cursor-pointer transition-all appearance-none rounded shadow hover:shadow-md border border-slate-300 checked:bg-cyan-300 checked:border-cyan-400" id="check1" />
